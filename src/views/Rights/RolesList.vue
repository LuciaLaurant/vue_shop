        ...
        <el-table-column type="expand">
          <template slot-scope="scope">
            <el-row :class="['bdbottom',i1 === 0?'bdtop':'','vcenter']" v-for="(item1,i1) in scope.row.children"
              ...
              <!-- 第一列 渲染一级权限 -->
              <el-col :span="5">
                <el-tag closable @close="removeRightNyId(scope.row,item1.id)">{{item1.authName}}</el-tag>
              ...
              <el-col :span="19">
                <el-row :class="[i2 === 0?'':'bdtop','vcenter']" v-for="(item2,i2) in item1.children" :key="item2.id">
                  <el-col :span="6">
                    <el-tag closable type="success" @close="removeRightNyId(scope.row,item2.id)">{{item2.authName}}
                    </el-tag>
                    <i class="el-icon-caret-right"></i>
                  </el-col>
                  <el-col :span="18">
                    <el-tag closable type="warning" v-for="(item3,i3) in item2.children" :key="item3.id"
                      @close="removeRightNyId(scope.row,item3.id)">
                      {{item3.authName}}
                    </el-tag>
                  </el-col>
                </el-row>
...
export default {
  ...
  methods: {
    ...
    // 删除权限
    async removeRightNyId(role, rightId) {
